...
const Library: React.FC<LibraryProps> = ({
  ...
  const allStrainCards = allStrains?.map((strain) => (
    <CannaCard
      key={strain.id}
      {...strain}
      type={card}
  ...
  const favoritedCards = favorites?.map((strain) => (
    <CannaCard
      key={strain.id}
      {...strain}
      type={card}
  ...
  const determineRender = () => {
    if (card === "Hybrids") {
      ...
      return favorites.length > 0 ? (
        ...
        <p>You haven't favorited any strains.</p>
      );
    }
  };
  ...
  return (
    <>
      ...
      <section className="all-strains-container">{determineRender()}</section>
    </>
  );
};
